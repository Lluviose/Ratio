...
- [x] **P1-9 配额耗尽主动处理**（`storageKernel.ts:297-330`）（2026-07-19 完成：失败退避重试 1s→30s、配额错误分级提示 + 一键清理本机快照自救、启动配额水位检查 >90% 告警）
...
- [x] **P2-13 升级 vite-plugin-pwa 0.21 → 1.x**（2026-07-19 完成：升 1.3.0（peer 含 Vite 7），五个懒屏幕依赖 chunk 进 `globIgnores` + 运行时 CacheFirst，check:bundle 新增「懒块不得进 precache」门禁）
...
- [ ] **P4-21 补 server 测试 + 管理员失败锁定**（安全三项完成 2026-07-23：管理员按 IP 失败锁定 + 失败审计、用户锁定 DoS 缓解（IP 维度锁定键 + 缓存命中豁免）、health 限流 + 探测缓存；集成测试 5 → 8 例。`adminConsole.js` 专项测试仍欠）
...
- [ ] **P4-23 工程化小项**（2026-07-19 部分：PROJECT.md 已补记视觉回归套件与写路径 e2e）

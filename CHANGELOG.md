...
## 2026-09-18 - 删除新购物品可选择回滚交易

- 非零净值、仅一次开户购入的物品，直接删除时可选择回滚购入、仅删除物品或取消；首次原值记录随购入一起清理，普通删除保留交易历史。
- 购入回滚沿用余额校准与非负余额保护，付款账户缺失或已归档时禁止回滚；信用卡等负债账户按原交易差额减少欠款。
- 修复购入识别忽略其他原值历史的问题，避免删除交易时连带删除有原值修正的物品；仅识别有效的零净值转入作为开户购入。
- 补充删除分支、归档、校准、负债及原值修正的回归测试，以及真实页面删除与 IndexedDB 持久化测试。
...
## 2026-08-23 - 通知样式 + 系统玻璃开关打坏自动备份


- **toast 套了系统液态玻璃**：开关打开后通知背景被掏空，字浮在一层雾上。设置文案只覆盖卡片/抽屉/导航，toast 改回原来的 CSS 毛玻璃。
- **自动备份被新设置项打成冲突**：`ratio.systemGlass` 进了备份文件，升级后本机多出一个键，和旧云端备份比对失败，自动同步标 conflict、弹出「自动备份暂停」。它是本机壳外观，排除出备份与脏标记；比对时也忽略已经躺在旧云端备份里的这个键，已冲突的设备下次同步能自己恢复。

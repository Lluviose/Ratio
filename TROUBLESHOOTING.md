...
## iOS 系统玻璃：悬浮底栏出现双重胶囊轮廓

系统材质已有胶囊轮廓和阴影，网页 `.navBar` 再叠 CSS 边框、内高光和
外阴影，会在材质中看到第二圈胶囊。开启系统玻璃时去掉这三项网页装饰，
由系统材质单独绘制底栏外观；移除边框的 1px 补入 padding，按钮位置和
安全区避让不变。当前标签的局部选中背景保留。

## iOS 系统玻璃：趋势图加载时黑灰闪现 / 四栏导航未悬浮

趋势骨架有主题实底，但真实图表、目标和详情面板仍曾套用系统材质，
在带透明度/位移入场的父层中初始化时会出现黑灰背景。修复覆盖真实面板：
保留主题底色、高光和图表动画，移除这些数据面板的系统材质及 backdrop 采样。
导航等 chrome 继续使用系统液态玻璃；开启且受支持时四栏导航为悬浮胶囊，
底部空隙避让 Home Indicator，关闭后恢复贴边布局。

`e2e/system-glass.spec.ts` 检查浅/深色面板的不透明底、窄屏胶囊几何和导航切换。
普通 Playwright 浏览器无法开启 WKWebView 私有系统材质，真机仍需检查冷启动和切页效果。
...
- 真机出现更窄的残余窗口：服务端写入后 iOS 直接终止页面，catch 没有机会回查；下次启动又先生成当天快照，远端已落盘的旧内容与当前本地不再相同，8 月的逐内容自愈无法认领。
- 每次自动 PUT 前先把 `ratio.cloudSync.pendingUpload` 检查点写入存储内核并 `await storageKernel.flush()`；检查点记录目标账号、旧基线、`backup.createdAt`、项目数和当时的 dirty token，且因 `ratio.cloudSync` 前缀不会进入备份或触发新的同步。
- 重启后先查远端 meta：`clientCreatedAt` 与项目数命中检查点才收编远端 `updatedAt`。只清除检查点记录的旧 dirty token；若启动后已有新快照/账户操作，新 token 保留，并立即基于刚收编的版本继续上传。远端未移动或身份不匹配时丢弃检查点，仍走原有重试/冲突保护。
- 升级前已经进入 conflict 且没有检查点的设备仍需人工解一次；新逻辑不凭设备名或时间猜测旧冲突归属。

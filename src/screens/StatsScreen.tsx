...
import { CalendarDays, ChevronDown, Info, Pencil, RotateCcw, Save, Sparkles, Target, Wallet, X } from 'lucide-react'
...
const CONFIDENCE_META: Record<DisposableConfidence, { short: string; label: string; tone: string }> = {
  high: { short: '高', label: '估算可信度较高', tone: '#10b981' },
  medium: { short: '中', label: '估算可信度中等', tone: '#f59e0b' },
  low: { short: '低', label: '记录较少 · 仅供参考', tone: '#f97316' },
...
function DisposableFundsCard(props: {
  snapshots: Snapshot[]
  ...
  const { snapshots, accountOps, summary, latestSnapshot, monthStartDay, paceAlgorithm, manualIncome, color, onChangeIncome } = props
  const estimate = useMemo(
    ...
    [snapshots, accountOps, summary, monthStartDay, paceAlgorithm, manualIncome, latestSnapshot],
  )
      ...
      <div className="cardInner" style={{ position: 'relative' }}>
            ...
            <div style={{ minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                ...
                <button
                  ...
                  aria-label="查看可支配估算说明"
                  aria-expanded={explainOpen}
                  aria-controls="disposable-explain"
                  title="查看可支配估算说明"
                  style={{
                    width: 22,
                    height: 22,
        ...
        <div style={{ marginTop: 12 }}>
          ...
          {estimate.limitedByLiquidity ? (
            <div style={{ marginTop: 6, fontSize: 11, fontWeight: 700, color: '#f97316' }}>
          ...
          <div style={{ marginTop: 14, display: 'grid', gap: 8 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, fontSize: 11, fontWeight: 650, flexWrap: 'wrap' }}>
              ...
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
                ...
                <span style={{ color: shortfallPart > 0 ? '#ef4444' : color }}>
        ...
        <MetricGrid layout="compactTop14">
          ...
          <MetricTile
            ...
            valueColor={estimate.monthlySurplus == null ? 'var(--muted-text)' : estimate.monthlySurplus >= 0 ? '#10b981' : '#ef4444'}
            sub="净资产月均增长"
          />
          <MetricTile
            compact

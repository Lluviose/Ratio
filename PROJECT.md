...
- 分包用显式 `advancedChunks` groups（`ai-assistant`、`screen-trend`、`screen-stats`（含 `screens/stats/`）、`screen-settings`、`vendor-markdown`（react-markdown 全家桶）、`vendor-matter`（matter-js，物理引擎按需加载））。**不要改回函数式 manualChunks，并保持 `includeDependenciesRecursively: false`**：rolldown 默认递归吸附依赖；若未显式关闭，入口会反向静态 import 名义上的懒块，浏览器首开实际把 AI/趋势/统计/设置/markdown 全部下载。vendor 组 priority 高于屏幕组，确保 vendor 先认领依赖。**vendor 组的 test 正则必须覆盖对应依赖树的完整传递闭包**（`includeDependenciesRecursively: false` 意味着漏网模块会落进消费方分包，与 vendor 分包互相 import 成环，先求值的一侧拿到未初始化绑定直接 TypeError——见 TROUBLESHOOTING「vendor 分包环路」）。`npm run check:bundle` 会检查六个懒块存在、gzip 预算、入口无静态反向引用及 vendor 分包无 import 环路；改分包后必须先 build 再跑该门禁。
...
直接删除净值非零且只有一次开户购入的物品时，提供「删除并回滚交易 / 仅删除物品 / 取消」。购入附带的首次原值记录视为同一次新建；有其他操作时不提供快捷回滚。回滚会按转账前后差额恢复付款账户（负债账户减少欠款），并删除购入和附属原值记录；付款账户缺失、已归档、后续已校准或回滚会造成负余额时禁用。仅删除物品保留历史、不改其他账户余额。手工新建但没有购入转账的物品也只删除条目。`accountCost.findItemOpenedByTransfer` 同时保护后续原值修正，不能将承载其他原值历史的物品当成单次购入一并删除。
...
- `main.tsx` 等 `storageKernel.ready` 后才挂载 React，组件树内的同步读一定读到权威数据；挂载前还会执行数据 schema 迁移（`src/lib/schemaVersion.ts`，失败/版本超前不阻断启动，只提示用户）。`openDb` 带超时（默认 5s）：IDB open 挂死（WebKit 已知缺陷）时按不可用回退 local，ready 不会悬挂白屏。
- 单实例守卫（`src/lib/instanceGuard.ts`）：同一时间只允许一个标签页读写——首个标签持有永不释放的 Web Locks 排他锁，后开标签停在拦截页、可 steal 接管；被接管方 flush 后整页冻结。这是对「键级 last-write-wins 下跨标签并发写丢更新」的结构性防护；`navigator.locks` 不存在时降级为无守卫。锁按 origin 隔离，Playwright 独立 context 的既有 e2e 不受影响（双标签用例见 `e2e/instance-guard.spec.ts`）。
...
- 落盘可靠性：失败的写入批次**不会被丢弃**，条目留在待写队列由后续任意 flush 自动重试，且失败后按退避（1s 翻倍至 30s，成功复位）主动重试，不依赖下一次用户写入；写失败会先重开一次连接再试（iOS 挂起恢复后连接被系统关闭的场景，`onclose` 后由写入路径惰性重连）。配额耗尽（QuotaExceededError）时提示升级为可操作引导：一键清理本机快照代际释放空间并立即重试；启动时还会做配额水位检查（>90% 提前告警）。IDB 本应可用却回退的会话会警示用户，且写入会在 localStorage 打降级标记，下次正常启动由 `localBackups.importFallbackSessionSnapshot()` 把降级期间的数据另存为本机快照。
...
- 与源码同目录，命名 `*.test.ts(x)`；全局 setup 在 `src/test/setup.ts`；`vitest.config.ts` 排除 `e2e/` 与独立的 `server/` Node 测试，并固定 `maxWorkers: 2` 避免 Windows 高负载 fork 冻结。
